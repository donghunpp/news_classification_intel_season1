...
# titles.append(re.compile('[^가-힝|a-z|A-Z]').sub(' ',title_tag.text))
# print(titles)
# print(len(titles))

df_titles = pd.DataFrame()
re_title = re.compile('[^가-힝|a-z|A-Z]')

for i in range(6):
    resp = requests.get('https://news.naver.com/main/main.naver?mode=LSD&mid=shm&sid1=10{}'.format(i), headers= headers )
    soup = BeautifulSoup(resp.text, 'html.parser')      # html로 변환
    title_tags = soup.select('.sh_text_headline')               #  headline만 뽑아냄
    titles = []
    for title_tag in title_tags:
        titles.append(re.compile('[^가-힝|a-z|A-Z]').sub(' ', title_tag.text))
